...
#### 方式A：PIL 数据可视化图（推荐用于数据主导的文章）
PIL 生成的图无 AI 水印，且数据展示准确。
...
**PIL配图脚本模板**：参考同目录下 `scripts/gen_charts_template.py`。

**设计风格**（默认暗色科技风）：
- 深蓝/深灰渐变背景 `(8,12,28)→(20,30,55)`
- 亮色文字：白色/浅蓝/青绿色
- 强调色：橙红 `(255,100,50)`、翠绿 `(80,200,120)`、金色 `(255,200,80)`

#### 方式B：AI 生图（ImageGen / DALL-E）→ 必须先去水印！
⚠️ **AI 生成的图右下角自带"图片由AI生成"水印，推送前必须裁剪掉底部！**

**去水印流程**：
```bash
# 裁剪底部 100px，去掉水印
python scripts/crop_watermark.py --dir {图片目录} --height 100
```

> **为什么不遮挡而是裁剪？** 用纯色填充覆盖水印会在图片上留下难看色块，比水印更显眼。直接裁剪底部 100px 干净无痕。

**注意**：AI 生图可能有文字错误，适合场景图/氛围图，不适合数据图表。
...
**关键参数说明**：
- `--no-watermark`：图片已经去水印（PIL生成无此水印，或AI图已裁剪过），加这个跳过服务端sharp处理
...
| AI图去水印脚本 | `scripts/crop_watermark.py` | 裁剪AI生成图底部100px去水印 |
...
- **AI 生图必须先去水印再推送**：用 `python scripts/crop_watermark.py --dir <图片目录>` 裁剪底部，切忌用纯色填充遮挡
